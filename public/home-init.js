...
(function(){
  ...
  st.textContent='.nav-peek{position:fixed;top:max(10px,env(safe-area-inset-top));left:14px;z-index:60;width:44px;height:44px;padding:1px;overflow:hidden;border:0;border-radius:50%;background:rgba(255,255,255,.58);backdrop-filter:blur(9px);-webkit-backdrop-filter:blur(9px);box-shadow:0 5px 16px -8px rgba(16,24,47,.4);opacity:0;transform:scale(.82) translateY(-6px);transform-origin:center;pointer-events:none;transition:opacity .28s ease,transform .42s cubic-bezier(.16,1,.3,1),box-shadow .42s ease;cursor:pointer}.nav-peek.show{opacity:1;transform:scale(1) translateY(0);pointer-events:auto}.nav-peek.show:hover{transform:scale(1.2) translateY(0);box-shadow:0 12px 26px -12px rgba(16,24,47,.46)}.nav-peek img{width:100%;height:100%;object-fit:contain;display:block;transform:scale(1.32);transform-origin:center;will-change:transform}.nav-peek.show:not(:hover) img{animation:navPeekIdleCycle 30s linear infinite}.nav-peek.show:hover img{animation:navPeekHoverSpin 1.45s cubic-bezier(.22,.61,.36,1) 1}@keyframes navPeekHoverSpin{from{transform:scale(1.32) rotate(0deg)}to{transform:scale(1.32) rotate(1080deg)}}@keyframes navPeekIdleCycle{0%,93.666%{transform:scale(1.32) rotate(0deg)}100%{transform:scale(1.32) rotate(1080deg)}}@media (prefers-reduced-motion:reduce){.nav-peek,.nav-peek.show:hover{transition:opacity .2s ease;transform:scale(1) translateY(0)}.nav-peek img{animation:none!important;transform:scale(1.32);will-change:auto}}';
